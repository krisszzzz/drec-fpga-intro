...
START_SGNL_MAPPED_ADDR = 0x00
IS_B_MAPPED_ADDR       = 0x04
ADDR_AB_MAPPED_ADDR    = 0x08
ADDR_C_MAPPED_ADDR     = 0x0c
...
class SystolicArrayTestbench:
    ...
    def setup_axil_master(self):
        ...
        self.log.info("AXI Lite initialized")
    ...
    async def reset(self):
        ...
        resp_start = await self.axil.write(START_SGNL_MAPPED_ADDR, 0x0.to_bytes(4, 'little'))
        assert resp_start.resp == 0, f"Expected OKAY (0), got {resp_start.resp}"

        # Set base addresses
        resp_ab = await self.axil.write(ADDR_AB_MAPPED_ADDR, 0x1000.to_bytes(4, 'little'))
        assert resp_ab.resp == 0, f"Expected OKAY (0), got {resp_ab.resp}"
        resp_c = await self.axil.write(ADDR_C_MAPPED_ADDR, 0x2000.to_bytes(4, 'little'))
    ...
    async def test_load_matrix_b(self, start):
        ...
        resp_is_b = await self.axil.write(IS_B_MAPPED_ADDR, 0x1.to_bytes(4, 'little'))
        assert resp_is_b.resp == 0, f"Expected OKAY (0), got {resp_is_b.resp}"
        ...
        # Start loading B
        resp_start = await self.axil.write(START_SGNL_MAPPED_ADDR, 0x1.to_bytes(4, 'little'))
        assert resp_start.resp == 0, f"Expected OKAY (0), got {resp_start.resp}"
        resp_start = await self.axil.write(START_SGNL_MAPPED_ADDR, 0x0.to_bytes(4, 'little'))
        assert resp_start.resp == 0, f"Expected OKAY (0), got {resp_start.resp}"
    ...
    async def test_load_matrix_a_and_save_c(self, start):
        ...
        resp_is_b = await self.axil.write(IS_B_MAPPED_ADDR, 0x0.to_bytes(4, 'little'))
        assert resp_is_b.resp == 0, f"Expected OKAY (0), got {resp_is_b.resp}"
